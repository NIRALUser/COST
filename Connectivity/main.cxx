...
int main(int argc, char * argv[])
{
  ...
  const char * file_basis_image = file_basis_image_str.c_str();//argv [1];//Input file
  const char * file_final = file_final_str.c_str();//argv[2];//Output file
  const char * file_input_label = file_input_label_str.c_str();//argv[3];//Label file
  
  short icosahedronSubdivision = icosahedronSubdivision_int;// ( argv[4] ) ;//Icosahedron subdivision level
  double thresholdCoefficient = thresholdCoefficient_float; //= atof ( argv[5] ) ;
  short vector_or_odf = vector_or_odf_int;//atoi ( argv[6] ) ;
  short prob_or_cost = prob_or_cost_int; //atoi ( argv[7] ) ; 
