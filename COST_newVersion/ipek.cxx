...
void ODFCost::VisitVoxel ( unsigned long x, unsigned long y, unsigned long z )
{
  ...
  for ( unsigned short n = 0 ; n < this->m_fstar_updates[this->m_fstar_direction][0] ; n++ )
    {
      ...
      neighborIndex = this->GetNeighbor ( x, y, z, neighborIterator, validNeighbor ) ; // this can be made into a table
  ...
  unsigned int origDir =  this->m_OrigDir[ neighborIndex ];
  unsigned int origNeighbor = this->m_OrigNeighbor[neighborIndex ];
  unsigned int origInDir = this->m_OrigInDir[ neighborIndex ];
  unsigned int origOutDir = this->m_OrigOutDir[ neighborIndex];
  ...
  double cost = this->m_NeighborAnglePenaltyTable[inDir][neighborIndex] + this->m_NeighborAnglePenaltyTable[outDir][neighborIndex]+this->m_AnglePenaltyTable[inDir][outDir]+preAngleCost;

		newStepCost = dn * ( this->m_alpha*f_odf + cost );
